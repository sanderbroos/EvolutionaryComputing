...
if __name__ == '__main__':
    ...
    if run_mode == 'test':
        ...
        for run in range(runs):
            ...
            bsol = np.loadtxt(f'group_2/{experiment_name}/solution/solution_run_{run}.txt')
        ...
        if not os.path.exists(f'group_2/{experiment_name}/solution/'):
            os.makedirs(f'group_2/{experiment_name}/solution/')

        # with open(f'group_2/{experiment_name}/solution/Gain',"w") as f:
        #     for (Gains,stat_p,stat_e) in zip(Gains,stat_p,stat_e):
        #         f.write("{0},{1},{2}\n".format(Gains,stat_p,stat_e))
        np.savetxt(f'group_2/{experiment_name}/solution/Gain.txt', Gains)
        np.savetxt(f'group_2/{experiment_name}/solution/Plife.txt', stat_p)
        np.savetxt(f'group_2/{experiment_name}/solution/Elife.txt', stat_e)
    ...
    for run in range(runs):
        ...
        if not os.path.exists(f'group_2/{experiment_name}/logbook'):
            os.makedirs(f'group_2/{experiment_name}/logbook')

        if not os.path.exists(f'group_2/{experiment_name}/solution'):
            os.makedirs(f'group_2/{experiment_name}/solution')

        # save logBook
        with open(f'group_2/{experiment_name}/logbook/logBook_run_{run}.pkl', 'wb') as f:
            pickle.dump(logbook, f)
        
        # select the best solution
        top = tools.selBest(pop, k=1)
        np.savetxt(f'group_2/{experiment_name}/solution/solution_run_{run}.txt', top)
