...
groups = [[2,4], [2,6], [7,8], [2,5,1], [2,5,6], [1,5,6], [4,6,7]]
group_id = 1
...
env = environment(group=groups[group_id], mode=run_mode)
...
if __name__ == '__main__':
    ...
    if run_mode == 'test':
        ...
        for run in range(runs):
            ...
            bsol = np.loadtxt(f'group_results/group_{group_id+1}/solution_run_{run}.txt')
        ...
        with open(f'group_results/group_{group_id+1}/GAIN.txt',"w") as f:
    ...
    for run in range(runs):
        ...
        if not os.path.exists(f'group_results/group_{group_id+1}'):
            os.makedirs(f'group_results/group_{group_id+1}')

        # save logBook
        with open(f'group_results/group_{group_id+1}/logBook_run_{run}.pkl', 'wb') as f:
            pickle.dump(logbook, f)
        
        # select the best solution
        top = tools.selBest(pop, k=1)
        np.savetxt(f'group_results/group_{group_id+1}/solution_run_{run}.txt', top)
