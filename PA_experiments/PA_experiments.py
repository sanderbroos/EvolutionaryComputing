...
if __name__ == '__main__':
    ...
    if run_mode == 'test':
        ...
        for eta_SBX in eta_SBXs:
            for eta_PM in eta_PMs:
                ...
                for run in range(runs):
                    ...
                    bsol = np.loadtxt(f'PA_experiments/solution/{experiment_name}/sbx_{eta_SBX}_pm_{eta_PM}/solution_run_{run}.txt')
                    ...
                    stat_p.append(playerlife)
                    stat_e.append(enemylife)
    ...
    for run in range(runs):
        ...
        for eta_SBX in eta_SBXs:
            for eta_PM in eta_PMs:
                ...
                if not os.path.exists(f'PA_experiments/logBook/{experiment_name}/sbx_{eta_SBX}_pm_{eta_PM}'):
                    os.makedirs(f'PA_experiments/logBook/{experiment_name}/sbx_{eta_SBX}_pm_{eta_PM}')

                if not os.path.exists(f'PA_experiments/solution/{experiment_name}/sbx_{eta_SBX}_pm_{eta_PM}'):
                    os.makedirs(f'PA_experiments/solution/{experiment_name}/sbx_{eta_SBX}_pm_{eta_PM}')

                # save logBook
                with open(f'PA_experiments/logBook/{experiment_name}/sbx_{eta_SBX}_pm_{eta_PM}/logBook_run_{run}.pkl', 'wb') as f:
                    pickle.dump(logbook, f)
                
                # select the best solution
                top = tools.selBest(pop, k=1)
                np.savetxt(f'PA_experiments/solution/{experiment_name}/sbx_{eta_SBX}_pm_{eta_PM}/solution_run_{run}.txt', top)
